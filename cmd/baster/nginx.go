...
func reloadNginxConfig(cnf *config.Config) error {
	nginxConf, err := generateConfig(cnf)
	if err != nil {
...
var servicesTemplate = template.Must(template.New("services").Parse(`
	{{range .}}
		server {
			listen 80;
			include errors.conf;
...
func generateConfig(cnf *config.Config) (string, error) {
	...
	if err := servicesTemplate.Execute(services, cnf.Services); err != nil {
		return "", errors.Trace(err)
	}
